...
PRE-ALGORITHM       How would a child solve this, you can write steps that have brute force, then see which steps can be optimized
...
1. gen new stack for each item pulled
2. update the input_arr with that removed item
...
def permute(c_arr, r_arr):
    ...
    for c in r_arr:
        c_arr.append(c)
        permute(c_arr, [x for x in r_arr if x != c])
        c_arr.pop()
...
arr = [{'name': 'a', 'age': 4}, {'name': 'b', 'age': 10}]
arr.sort(key=itemgetter('age'))
...
idx2 = "asdfasdf".rfind('f', 0, len("asdfasdf"))
...
mst_cmn = Counter(arr).most_common(2)[0]  # 2 most common entries
...
s = "asdf"
sub_str = [s[i:j + 1] for i in range(len(s)) for j in range(i, len(s))]
...
dct = {}  # this is a declaration of a dict, not a set!
st = {3}  # this is the definition of a set!
